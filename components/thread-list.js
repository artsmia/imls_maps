...
class ThreadList extends React.Component {
  render() {
    ...
    const threadBorderColorStyles = threads
      .map(
        (thread, index) => `
      ...
      <ul>
        {threads.map((thread, index) => {
          const threadIsActive =
            activeThread && activeThread.basename == thread.basename
          if (threadIsActive)
            console.info('ThreadList', {
              thread,
              threadIsActive,
              activeThread: activeThread,
            })
          const threadElem = (
            <span onClick={activateThread.bind(this, thread)}>
              {thread.title}
            </span>
          )
          ...
          return (
            <li
              ...
              key={thread.title}
              style={{ opacity: !activeThread || threadIsActive ? '1' : '0.5' }}
            >
              {threadIsActive ? (
...
class QuickFacts extends React.Component {
  ...
  render() {
    ...
    const videoFile =
      ...
      (thread.title === 'Asian Design and Influence' &&
        'asianDesign_final_8.8.mp4') ||
      (thread.title === 'Blue and White Ceramics' &&
        'blueWhite_final_8.8.mp4') ||
    ...
    const activateFirstArtwork = () =>
      this.props.setGlobalState({ activeArtwork: thread.artworks[0] })
    ...
    return (
      <div style={{ backgroundColor: 'white', padding: '0.5em' }}>
        ...
        <p
          style={{ marginBottom: 0, fontFamily: 'MiaGrotesk-Light' }}
          onTouchEnd={nextFact}
        >
          {activeFact}
        </p>
        <div style={{ paddingTop: '0.5em' }}>
          {factSelectors}
          <a style={{ color: 'red', marginLeft: '1em' }} onClick={nextFact}>
            See next &rarr;
          </a>
        </div>
        <p onClick={activateFirstArtwork}>
          Select an artwork on the map to begin.
        </p>

        {showVideo && (
          <div style={videoWrapperStyles} onClick={this.stopVideo.bind(this)}>
